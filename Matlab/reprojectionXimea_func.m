function [] = reprojectionXimea_func( pcName, cameraParam, cam0yaml, cam1yaml, exFile, ...
...
images = uint8(zeros(imageHeight1,imageWidth1,bands(end),NoF));
...
for batchesIt = 1:NoB
    ...
    for iBand = bands
        for pointIt = 1:totalPoints
            tempColor(points(pointIt),1,iBand) = tempColor(points(pointIt),1,iBand) + double(images(v(pointIt),u(pointIt),iBand,frame(pointIt)));
        end
    end
