classdef VISImage
    ...
    properties
        ...
        % No filter
        wavelengthsRowMajor = [465, 546, 586, 630,...
            474, 534, 578, 624,...
            485, 522, 562, 608,...
            496, 510, 548, 600];
    ...
    methods
        ...
        function obj = VISImage(img, varargin)
            if nargin > 0
                obj.rawImage = img;
                ...
                if numvarargs > 1
                    error('VISImage(img,exposureTime): Too many input arguments');
                end
        ...
        %         function obj = VISImage(img, wavelengthsRowMajor, offset_r = 3, offset_c = 0, width = 2045, height = 1080, blksize = 5 )
        %             obj.rawImage = img;
        ...
        function obj = extractDataCube(obj)
            ...
            for i = 1:obj.blksize % blkrow
                for j = 1:obj.blksize % blkcol
                    obj.dataCube(:,:,(i-1)*obj.blksize+j) = img(i:4:end,j:4:end);
                end
            end
            
            %% Sort in order of increasing wavelength to make spectral variation plots easier
            [obj.wavelengthsRowMajor, sortedIndices] = sort(obj.wavelengthsRowMajor);
            sortedDataCube = zeros(size(obj.dataCube));
            for iBand = 1:obj.nBands
                sortedDataCube(:,:,iBand) = obj.dataCube(:,:, sortedIndices(iBand));
            end
            obj.dataCube = sortedDataCube;
        end
        ...
        function obj = extractDataCubeOriginalFormat(obj)
            ...
            for i = 1:obj.blksize % blkrow
                for j = 1:obj.blksize % blkcol
                    obj.dataCubeOriginalFormat(:,:,(i-1)*obj.blksize+j) = img(i:4:end,j:4:end);
                end
            end
        ...
        function obj = createResponseCorrectedCube(obj, inverseSensorResponse, darkNoiseImg)
            obj.responseCorrectedCube = VISImage(inverseSensorResponse(1+obj.rawImage) - darkNoiseImg).dataCube;
        ...
        function obj = createCalibratedDataCube(obj, spectralComponent)
            ...
            obj.calibratedDataCube = rescale(obj.calibratedDataCube,[0 1]);
        ...
        function fh = inspect(obj, method)
            if isequal(method, 'calibrated')
                fh = im_cube_slicer(obj.calibratedDataCube, obj.wavelengthsRowMajor, 1, 'fixed')
            else if isequal(method, 'raw')
                    fh = im_cube_slicer(obj.dataCube, obj.wavelengthsRowMajor, 1, 'fixed')
                end
            end
        end
        ...
        function saveDataCube(obj,folderName,idxImg)
            for i = 1:obj.blksize % blkrow
                for j = 1:obj.blksize % blkcol
                    ...
                    img=obj.dataCube(:,:,(i-1)*obj.blksize+j);
        ...
        function saveCalibratedDataCube(obj,folderName,idxImg)
            if (obj.isCalibrated)
                for i = 1:obj.blksize % blkrow
                    for j = 1:obj.blksize % blkcol
                        name=num2str(obj.wavelengthsRowMajor((i-1)*obj.blksize+j));
                        ...
                        [~,~,~]=mkdir([folderName '/' strImg]);
                        file_path_with_name=[folderName '/' strImg '/' name '.png'];
                        img=obj.calibratedDataCube(2:end-1,2:end-1,(i-1)*obj.blksize+j);
                        img=img-min(img(:)); %shift data
                        img=img/max(img(:)); %normalize to 1
                        imwrite(img,file_path_with_name,'BitDepth', 16);
                    end
                ...
                disp('Error: Image not calibrated');
            end    
        end
