...
% IMPROVED/SIMPLIFIED SETUP FOR NNET.MODE.MATLAB
% ==============================================
% WILL BE FURTHER STREAMLINED AND USED TO REPLACE OLD CODE IN MODE CLEANUP TASK
if strcmp(calcMode.mode,'nnet.mode.Matlab')
  [calcMode,calcNet,calcData,calcHints,resourceText] = iSimplerSetup(calcMode,net,data,showResources);
  return
end
...
if ~strcmp(calcMode.mode,'nnParallel')
  ...
  doPd = iPredelayedProcessedInputs(calcMode,net);
  calcHints.doFlattenTime = iShouldFlattenTime(net,calcHints,doPd);
...
if ~isComposite
  ...
  spmd
    ...
    if ~isempty(workerModes)
      ...
      doPd = ~isa(data.X,'gpuArray') && iPredelayedProcessedInputs(workerModes,net);
      calcHints.subhints.flattenTime = iShouldFlattenTime(net,calcHints,doPd);
...
function flag = iModeSupportsPredelayedInputs(calcMode)
...
elseif isfield(calcMode.hints,'subcalc')
  flag = iModeSupportsPredelayedInputs(calcMode.hints.subcalc);
elseif isfield(calcMode.hints,'subcalcs')
  for i=1:numel(calcMode.hints.subcalcs)
    if ~iModeSupportsPredelayedInputs(calcMode.hints.subcalcs{i})
      flag = false;
      return
    end
  end
...
switch calcMode.mode
  ...
  case 'nnet.mode.Matlab'
    modeText = {['MATLAB on ' computer]};
  
  case 'nnMex'
    modeText = {['MEX on ' computer]};
    
  % Numerical approximations for testing
  ...
  % LEGACY MATLAB - Obsolete in future
    
  case {'nnMATLAB', 'nnSimple', 'nn7'}
    modeText = {['MATLAB on ' computer]};
...
% IMPROVED/SIMPLIFIED SETUP FOR NNET.MODE.MATLAB
% WILL BE FURTHER STREAMLINED AND USED TO REPLACE OLD CODE IN MODE CLEANUP TASK
function [mode,calcNet,calcData,calcHints,resourceText] = iSimplerSetup(mode,net,data,showResources)

% Calculation Network - Unchanged from provided network
calcNet = net;

% Calculation Data - Unchanged from provided data
calcData = data;

% Calculation Hints
calcHints = mode.hints;
calcHints.isComposite = false;
calcHints.isActiveWorker = true;
calcHints.doFlattenTime = false;
if strcmp(mode.options.useGPU,'yes')
  calcHints.useGPU = 'yes';
end

% Calculation Hints
% Performance hints to be simplified with performance measure integration
calcHints = mode.netHints(net,calcHints);
if isempty(net.performFcn)
  calcHints.perfNorm = false;
  calcHints.regularization = 0;
else
...
% ----- Calculation Mode
mode.isActiveWorker = true;
mode.isMainWorker = true;
mode.mainWorkerInd = 1;
mode.isParallel = false;

% ----- Show Resource Text
if showResources
  resourceText = mode2Text(mode,calcHints);
else
  resourceText = {};
end

function doPd = iPredelayedProcessedInputs(calcMode,net)
% iPredelayedProcessedInputs   True for Pre-delayed processed inputs 
doPd = (net.numInputDelays > 0) ...
  && (net.numLayerDelays == 0) ...
  && net.efficiency.cacheDelayedInputs ...
  && iModeSupportsPredelayedInputs(calcMode);

function tf = iShouldFlattenTime(net,calcHints,doPd)
% iShouldFlattenTime   True we should fallten time
tf = ...
  net.efficiency.flattenTime ...
  && (net.numLayerDelays==0) ...
  && (calcHints.TSu > 1) ...
  && ((net.numInputDelays==0) || doPd);
