...
%NETHINTS Useful neural network values for calcuations

% Copyright 2012-2015 The MathWorks, Inc.

net = struct(net);

  hints = iDimensionHints(hints,net);
  hints = iInputHints(hints,net);
  hints = iLayerHints(hints,net);
  hints = iOutputHints(hints,net);
  hints = iWeightHints(hints,net);
  hints = iPerformanceHints(hints,net);
  hints = nn.wb_indices(net,hints);
end

function hints = iDimensionHints(hints,net)
  % Inputs
...
function hints = iInputHints(hints,net)
...
function hints = iLayerHints(hints,net)
...
function hints = iOutputHints(hints,net)
  hints.numOutProc = [];
...
function hints = iWeightHints(hints,net)
  hints.numZ = [];
  for i=1:net.numLayers
    zInd = 0;
...
function hints = iPerformanceHints(hints,net)
