classdef LSTMLayer < nnet.cnn.layer.Layer & nnet.internal.cnn.layer.Externalizable
    ...
    %     OutputSize                          - Output size of the layer
    ...
    properties(SetAccess = private, Dependent)
        ...
        % ReturnSequence   A logical, which specifies the output format of
        % the layer. If true, output is a sequence. Otherwise output is the
    ...
    properties(Dependent)
        ...
        %   will have size 4*OutputSize-by-InputSize.
        InputWeights
        ...
        %   individual matrices are concatenated in the following order:
        %   input gate, forget gate, layer input, output gate. This matrix
        %   will have size 4*OutputSize-by-OutputSize.
        RecurrentWeights
        ...
        %   output gate. This vector will have size 4*OutputSize-by-1.
        Bias
    ...
    properties(SetAccess = private, Dependent)
        % OutputState   The initial value of the output state.
    ...
    methods
        ...
        function this = set.InputWeights(this, value)
            ...
            attributes = {'size', [4*this.OutputSize expectedInputSize], 'nonempty', 'real', 'nonsparse'};
        ...
        function this = set.RecurrentWeights(this, value)
            classes = {'single', 'double', 'gpuArray'};
            attributes = {'size', [4*this.OutputSize this.OutputSize], 'nonempty', 'real', 'nonsparse'};
            validateattributes(value, classes, attributes);
        ...
        function this = set.Bias(this, value)
            classes = {'single', 'double', 'gpuArray'};
            attributes = {'column', 'nonempty', 'real', 'nonsparse', 'nrows', 4*this.OutputSize};
            validateattributes(value, classes, attributes);
        ...
        function this = set.CellState(this, value)
            classes = {'single', 'double', 'gpuArray'};
            attributes = {'size', [this.OutputSize 1], 'nonempty', 'real', 'nonsparse'};
            validateattributes(value, classes, attributes);
        ...
        function this = set.OutputState(this, value)
            classes = {'single', 'double', 'gpuArray'};
            attributes = {'size', [this.OutputSize 1], 'nonempty', 'real', 'nonsparse'};
            validateattributes(value, classes, attributes);
        ...
        function out = saveobj(this)
            ...
            out.OutputSize = privateLayer.HiddenSize;
            ...
            out.CellState = gather(privateLayer.CellState);
            out.OutputState = gather(privateLayer.HiddenState);
    ...
    methods(Static)
        ...
        function this = loadobj(in)
            ...
            internalLayer.CellState = in.CellState;
            internalLayer.HiddenState = in.OutputState;
    ...
    methods(Hidden, Access = protected)
        ...
        function groups = getPropertyGroups( this )
            ...
            learnableParameters = { 'InputWeights', ...
                'RecurrentWeights', ...
                'Bias' };
            stateParameters = { 'OutputState', 'CellState' };
            groups = [
        ...
        function val = getFactor(this, val)
            if isscalar(val)
                % No operation needed
            elseif numel(val) == (4*this.OutputSize)
                val = val(1:this.OutputSize:end);
                val = val(:)';
            else
        ...
        function val = setFactor(this, val)
            if isscalar(val)
                % No operation needed
            elseif numel(val) == 4
                % Expand a four-element vector into a 4*OutputSize-by-1
                % column vector
                expandedValues = repelem( val, this.OutputSize );
                val = expandedValues(:);
            else
                % Error - the factor has incorrect qememorysize
            end
        end
    end
end
...
defaultBiasL2Factor = 1;

p.addRequired('OutputSize', @(x)validateattributes(x, {'numeric'}, {'scalar', 'positive', 'integer'}));
