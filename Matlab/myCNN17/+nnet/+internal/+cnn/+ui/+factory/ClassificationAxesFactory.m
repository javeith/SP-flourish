classdef ClassificationAxesFactory < nnet.internal.cnn.ui.factory.AxesFactory
    ...
    methods
        function [axesView, metrics] = createMainAxesAndMetrics(~, epochInfo, epochDisplayer) 
            ...
            metrics{1} = nnet.internal.cnn.ui.metric.Metric(trainingUpdateableSeries, 'Accuracy');
            metrics{2} = nnet.internal.cnn.ui.metric.Metric(validationUpdateableSeries, 'ValidationAccuracy');
        ...
        function [axesView, metrics] = createLossAxesAndMetrics(~, epochInfo, epochDisplayer)
            ...
            trainingUpdateableSeries = nnet.internal.cnn.ui.axes.UpdateableSeries();
            validationUpdateableSeries = nnet.internal.cnn.ui.axes.UpdateableSeries(); 
            metrics{1} = nnet.internal.cnn.ui.metric.Metric(trainingUpdateableSeries, 'Loss');
            metrics{2} = nnet.internal.cnn.ui.metric.Metric(validationUpdateableSeries, 'ValidationLoss');
...
function lineModel = iLineModel(series, name, lineStyle, lineColor, markerType, markerFaceColor, markerEdgeColor, markerStrategy)
