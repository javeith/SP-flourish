classdef CustomLayer < nnet.internal.cnn.layer.Layer
    ...
    methods
        ...
        function [dX, dW] = backward( this, X, Z, dZ, memory )
            ...
            try
                [dX, dW{1:this.NumParameters}] = backward( this.ExternalCustomLayer, X, Z, dZ, memory );
            catch cause
                iThrowWithCause(cause, 'nnet_cnn:internal:cnn:layer:CustomLayer:BackwardErrored', class(this.ExternalCustomLayer))
            end
            
            this.LayerVerifier.verifyBackwardSize( X, this.LearnableParameters, dX, dW );
            this.LayerVerifier.verifyBackwardType( X, dX, dW );
        end
