classdef LSTM < nnet.internal.cnn.layer.Layer ...
    ...
    methods
        ...
        function this = initializeLearnableParameters(this, precision)
            ...
            if isempty(this.InputWeights.Value)
                ...
                inputSize = [4*this.HiddenSize, this.InputSize];
            ...
            if isempty(this.RecurrentWeights.Value)
                % Initialize only if it is empty
                hiddenSize = [4*this.HiddenSize, this.HiddenSize];
...
parameter([zInd iInd fInd oInd], :) = precision.cast( ...
    [zeros(2*hiddenSize, 1); ones(hiddenSize, 1); zeros(hiddenSize, 1)] );
end
