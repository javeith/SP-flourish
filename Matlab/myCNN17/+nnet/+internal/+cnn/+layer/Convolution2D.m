classdef Convolution2D < nnet.internal.cnn.layer.Layer
    ...
    methods
        ...
        function [dX,dW] = backward( this, X, ~, dZ, ~ )
            ...
            % through the layer
            if(this.usingFilterGroups())
                [dX,dW] = this.backwardTwoFilterGroups(X, [], dZ, []);
            else
                [dX,dW] = this.backwardNormal(X, [], dZ, []);
            end
        end
    ...
    methods(Access = private)
        ...
        function [dX,dW] = backwardNormal(this, X, ~, dZ, ~)
            [dX,dW] = this.doBackward(X, this.Weights.Value, dZ);
        end
        ...
        function [dX,dW] = backwardTwoFilterGroups(this, X, ~, dZ, ~)
            ...
            [dX1,dW1] = this.doBackward(X1, weights1, dZ1);
            [dX2,dW2] = this.doBackward(X2, weights2, dZ2);
            
            dX = cat(3, dX1, dX2);
            dW = {cat(4, dW1{1}, dW2{1}), cat(3, dW1{2}, dW2{2})};
        end
        
        function [dX,dW] = doBackward(this, X, weights, dZ)
            % Note that padding is stored as [top bottom left right] but
            % the function expects [top left bottom right].
