...
% retaining all the underlying true information.
    ...
    methods( Static, Hidden )
        ...
        % code in the stack
            ...
            % TODO(g1616310): Datastore should retain original error
            % somewhere in this caught exception, so that this non-robust
            % string parsing is not necessary.
            if remotecause.identifier == "MATLAB:datastoreio:customreaddatastore:readFcnError"
                % Make sure there are at least four lines
                msg = sprintf('%s\n\n\n\n', remotecause.message);
                % Message is the line before the second empty line
                lines = regexp(msg, '\n');
                emptyLines = regexp(msg, '\n\n');
                msgStartNewlineIndex = find(lines == emptyLines(2), 1) - 1;
                msg = msg(lines(msgStartNewlineIndex)+1:lines(msgStartNewlineIndex+1)-1);
            end
