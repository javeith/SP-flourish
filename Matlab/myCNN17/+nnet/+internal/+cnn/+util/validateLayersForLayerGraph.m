...
% Assert validity of any custom layers in larray
iValidateCustomLayers(larray);
...
isLSTMLayer = arrayfun(@(x) iIsLSTMLayer(x), larray);
if any(isLSTMLayer)
    error(message('nnet_cnn:nnet:cnn:LayerGraph:LSTMLayerNotAllowed'));
end
...
function tf = iIsLSTMLayer(layer)
...
function iValidateCustomLayers(layers)
try
