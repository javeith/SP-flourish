...
if nargin == 1 && iInputIsDAGNetwork(varargin{1})
    ...
    % TODO: This package location will change in the final version
    lgraph = nnet.cnn.LayerGraph( ...
        inputArguments.Layers, ...
        inputArguments.Connections);
end
...
defaultConnections = 'series';

p.addOptional('Layers', defaultLayers, @(x)iValidateLayers(x));
p.addParameter('Connections', defaultConnections);
...
inputArguments.Connections = validatestring(results.Connections, iExpectedConnections());
end

function expectedConnections = iExpectedConnections()
expectedConnections = {'series', 'none'};
end