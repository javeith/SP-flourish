function layer = transposedConv2dLayer( varargin )
...
internalLayer = nnet.internal.cnn.layer.TransposedConvolution2D(args.Name, ...
    args.FilterSize, ...
    args.NumChannels, ...
    args.NumFilters, ...
    args.Stride, ...
...
inputArguments.Cropping = iMakeIntoRowVectorOfTwo(p.Results.Cropping);
...
inputArguments.Name = char(p.Results.Name); % store name as char vector.
end
...
nnet.internal.cnn.layer.paramvalidation.validateStride(value);
end
