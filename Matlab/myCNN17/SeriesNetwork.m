classdef SeriesNetwork
    ...
    methods(Access = private, Hidden)
        ...
        function this = SeriesNetworkFromLayersAndMap(this, internalLayers, layersMap)
            ...
            % Create categories beforehand
            classNames = iClassNames(this.PrivateNetwork);
            
            % Set the data to be all the categories
            this.Categories = categorical(classNames);
            
            % Set the appropriate categories (thus enforcing the original
            % order)
            this.Categories = setcats(this.Categories, classNames);
    ...
    methods(Access = public)
        ...
        function [this, labels, scores] = classifyAndUpdateState(this, X, varargin)
            ...
            %                                 CPU.
            %                               - 'gpu' - Use the GPU. To use a
            %                                 GPU, younnet.internal.cnn.layer.learnable.CacheHandle(); must have Parallel
            %                                 Computing Toolbox(TM), and a
            %                                 CUDA-enabled NVIDIA GPU with
            %                                 compute capability 3.0 or
            %                                 higher. If a suitable GPU is
        ...
        function Y = activations(this, X, layerID, varargin)
            ...
            %   be a matrix with one row per observation. See the
            %   'OutputAs' optional name-value pair for more details.
            %
    ...
    methods (Access=private)
        ...
        function [Y, finalState, predictNetwork] = predictRNN(this, data, ...
            ...
            if ~this.ReturnSequence
                ...
                Y = cell(dispatcher.NumObservations, 1);
...
if isprop(outputLayer, 'ClassNames')
...
try
    if isa(data,'nnet.internal.cnn.MiniBatchDatasource')
...
if isnumeric( X )
    % Single observation
    sInd = min( size( X, 2 ), size( P, 3 ) );
    Y = squeeze( P(:, 1, 1:sInd ) );
else
    % Multiple observations
    for ii = 1:numel(inds)
        sl = size( X{ inds(ii) }, 2 );
        sInd = min( sl, size( P, 3 ) );
        Y{ inds(ii) } = squeeze( P(:, ii, 1:sInd) );
    end
end
end
