...
corn1_pc = plyread([hddLoc 'thanujan/Datasets/xClassifier/trainSet/Corn1/NIR25/crop1_1_band1.ply']);
size1 = size(corn1_pc.Color,1);
cornX(1:size1) = corn1_pc.Location(:,1);
cornY(1:size1) = corn1_pc.Location(:,2);
clear corn1_pc;

corn2_pc = plyread([hddLoc 'thanujan/Datasets/xClassifier/trainSet/Corn2/NIR25/crop1_2_band1.ply']);
cornX((size1+1):(size1+size(corn2_pc.Color,1))) = corn2_pc.Location(:,1);
cornY((size1+1):(size1+size(corn2_pc.Color,1))) = corn2_pc.Location(:,2);
clear corn2_pc size1;

sugarBeet_pc = plyread([hddLoc 'thanujan/Datasets/xClassifier/trainSet/Sugarbeet/NIR25/crop2_1_band1.ply']);
sugarBeetX(:) = sugarBeet_pc.Location(:,1);
sugarBeetY(:) = sugarBeet_pc.Location(:,2);
clear sugarBeet_pc;

winterWheat1_pc = plyread([hddLoc 'thanujan/Datasets/xClassifier/trainSet/Winterwheat1/NIR25/crop3_1_band1.ply']);
size2 = size(winterWheat1_pc.Color,1);
winterWheatX(1:size2) = winterWheat1_pc.Location(:,1);
winterWheatY(1:size2) = winterWheat1_pc.Location(:,2);
clear winterWheat1_pc;

winterWheat2_pc = plyread([hddLoc 'thanujan/Datasets/xClassifier/trainSet/Winterwheat2/NIR25/winterwheat2_band1.ply']);
winterWheatX((size2+1):(size2+size(winterWheat2_pc.Color,1))) = winterWheat2_pc.Location(:,1);
winterWheatY((size2+1):(size2+size(winterWheat2_pc.Color,1))) = winterWheat2_pc.Location(:,2);
clear winterWheat2_pc size2;

buckWheat_pc = plyread([hddLoc 'thanujan/Datasets/xClassifier/trainSet/Buckwheat/NIR25/buckwheat_band1.ply']);
buckWheatX(:) = buckWheat_pc.Location(:,1);
buckWheatY(:) = buckWheat_pc.Location(:,2);
clear buckWheat_pc;

grass_pc = plyread([hddLoc 'thanujan/Datasets/xClassifier/trainSet/Grass/NIR25/grass_band1.ply']);
grassX(:) = grass_pc.Location(:,1);
grassY(:) = grass_pc.Location(:,2);
clear grass_pc;

road_pc = plyread([hddLoc 'thanujan/Datasets/xClassifier/trainSet/road/NIR25/road_band1.ply']);
roadX(:) = road_pc.Location(:,1);
roadY(:) = road_pc.Location(:,2);
clear road_pc;

soil_pc = plyread([hddLoc 'thanujan/Datasets/xClassifier/trainSet/soil/NIR25/soil_band1.ply']);
soilX(:) = soil_pc.Location(:,1);
soilY(:) = soil_pc.Location(:,2);
clear soil_pc;

soy_pc = plyread([hddLoc 'thanujan/Datasets/xClassifier/trainSet/Soybean/NIR25/soybean_band1.ply']);
soyBeanX(:) = soy_pc.Location(:,1);
soyBeanY(:) = soy_pc.Location(:,2);
clear soy_pc;
...
% matrix T.
%
% Each ith column of the input matrix will have the format 1x1x41:
% [Breadth X]x[Width Y]x[41 Bands]
%
...
% TRAINING 01:
x = double([soilX, roadX, buckWheatX, cornX, grassX, soyBeanX, sugarBeetX, winterWheatX;soilY, roadY, buckWheatY, cornY, grassY, soyBeanY,...
            sugarBeetY, winterWheatY; soil, road, buckWheat, corn, grass, soyBean, sugarBeet, winterWheat]);
