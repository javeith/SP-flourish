...
cam1yaml = '/home/thanu/Documents/CWG-CALTag2/shortBag/ximea_falcon.yaml';

% - Extrinsic parameters (.info or .yaml)
exFile = '/home/thanu/Documents/CWG-CALTag2/shortBag/DJIXimeaExtrinsic1img.yaml';

% - Corresponding gimbal angles for each ximea image (atm: .mat; should be integrated in this script later)
load /home/thanu/Documents/CWG-CALTag2/shortBag/YPRDJI.mat;
load /home/thanu/Documents/CWG-CALTag2/shortBag/T_gimbalAngles.mat;
...
% w2cTrafo(2:5:end,:) = w2cTrafo(2:5:end,:) ./ 1000;
...
% 
% for i = 1:NoF_1   
%     T_gimbalAngles(1:3,1:3,i) = eul2rotm([YPRXimea(i,1), YPRXimea(i,2), YPRXimea(i,3)]);
%     T_10_eachFrame(:,:,i) = T_gimbalAngles(:,:,i) * T_10;
% end

NoF_0 = size(w2cTrafo,1) / 5;
% T_gimbalAngles = zeros(4,4,NoF_0);
% T_gimbalAngles(4,4,:) = 1;
% T_10_eachFrame = T_gimbalAngles;

for i = 1:NoF_0  
%     T_gimbalAngles(1:3,1:3,i) = eul2rotm([degtorad(YPRDJI(i+14,2)), degtorad(YPRDJI(i+14,3)), degtorad(YPRDJI(i+14,4))]);
% T_gimbalAngles(1:3,1:3,i) = eye(3);
    T_10_eachFrame(:,:,i) = eye(4) * T_10;
%     T_10_eachFrame(:,:,i) = eye(4);
end
...
imageID = [193;205;217;229;245];
for ID = 1:1%1:NoF_1
%     imageID = ID - 1;
    if sequoia == 1
        
        filename = strcat([imagePath '/'],srcFiles(imageID(ID)+1).name);
        ...
        if imageID(ID) < 10
            filename = [imagePath '/frame000' num2str(imageID(ID)) '.jpg'];
            
        elseif imageID(ID) < 100
            filename = [imagePath '/frame00' num2str(imageID(ID)) '.jpg'];
            
        elseif imageID(ID) < 1000
            filename = [imagePath '/frame0' num2str(imageID(ID)) '.jpg'];
            
        elseif imageID(ID) < 10000
            filename = [imagePath '/frame' num2str(imageID(ID)) '.jpg'];
...
% rotationMatrix = repmat(T_10(1:3,1:3), 1, NoF_0);
% rotationMatrix = mat2cell(rotationMatrix, 3, repmat(3,1,NoF_0));
% rotationMatrix = blkdiag(rotationMatrix{:});

rotationMatrix = T_10_eachFrame(1:3,1:3,:);
rotationMatrix = permute(rotationMatrix,[1 3 2]);
rotationMatrix = reshape(rotationMatrix,[NoF_0*3,3]);
rotationMatrix = mat2cell(rotationMatrix, repmat(3,1,NoF_0), 3);
rotationMatrix = blkdiag(rotationMatrix{:});
% rotationMatrix = reshape(rotationMatrix,[NoF_0*3,3]);
...
%     P_c1 = rotationMatrix * P_c1 + repmat(T_10(1:3,4),NoF_0,1);
    t = reshape(T_10_eachFrame(1:3,4,:),[NoF_0*3,1]);
%     t = repmat(t,NoF_0,1);
...
%         p_hd = zeros(2*NoF_0*NoF_1, NoP_batch);
    ...
    p_c1 = focalMatrix * p_hd + repmat(cameraMatrix1(1:2,3),NoF_0,1);
